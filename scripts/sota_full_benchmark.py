...
report_file = f"{base_dir}/data/export/sota_full_benchmark_report.json"
...
print("Carregando corpus consolidado com expansão semântica...")
doc_ids = []
doc_texts = []
doc_lens = []
...
with open(corpus_file, "r", encoding="utf-8") as f:
    for idx, line in enumerate(f):
        ...
        # O corpus inclui o texto da claim + contexto + perguntas sintéticas
...
print(f"Índice SOTA construído para {N} documentos.")
...
def search_sota(query_raw, top_k=10, rerank_depth=40):
    ...
    # 2º Estágio: Reranker de precisão
    ...
    for doc_idx in candidates:
        ...
        for code in aws_codes:
            ...
            if code in text_lower or code_alt in text_lower:
                s += 25.0
            if code in cid_lower or code_alt in cid_lower:
                s += 45.0
...
print(f"Executando SOTA Benchmark em {total_eval} perguntas...")
...
print("\n" + "="*60)
print("        RELATÓRIO DO SOTA FULL BENCHMARK (3.180 PERGUNTAS)")
print("="*60)
